 ...
 * Cloudflare Pages Function: файл лежит в functions/api/lead.js,
 * значит сайт сам отдаёт его по адресу /api/lead — тот же домен,
 * никакого CORS, токен бота остаётся на сервере.
 *
 * Переменные (Pages → Settings → Variables and Secrets → Production):
 *   BOT_TOKEN  (Secret) — токен от @BotFather
 *   CHAT_ID    (Secret) — id чата. Несколько — через запятую: 111,222
 *
 * В assets/config.js тогда: endpoint: '/api/lead'
 */
...
export function onRequestGet() {
  return json({ ok: true, service: 'konga-leads' }, 200);
}
