...
popup: {
  ...
  repeatAfterHours: 24   // не показывать повторно столько часов; 0 — каждый раз
}
```
...
Закрыть можно крестиком, кликом по фону или Esc; повторно баннер не появится
`repeatAfterHours` часов (хранится в localStorage браузера посетителя).
